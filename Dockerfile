...
RUN make -s
## ignore modify /etc/httpd/conf/httpd.conf with apxs command.
## (local-php.rpm に /etc/httpd/conf/httpd.conf が含まれないようにする。httpdパッケージとconfrictする問題の回避)
RUN sed -i -e "s@/usr/sbin/apxs -S@echo /usr/sbin/apxs -S@g" Makefile
RUN make install
